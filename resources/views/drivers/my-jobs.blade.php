                ...
                <td>
                    <div class="btn-group" role="group">
                        <form method="post" action="{{ route('approvetxn', $pjob->txn_id) }}">
                            @csrf
                            <button type="submit" class="btn btn-success float-left m-auto" @if($pending!=1) disabled
                                @else @endif>
                        ...
                        <form method="post" action="{{ route('denytxn', $pjob->txn_id) }}">
                            @csrf
                            <button type="submit" class="btn btn-danger float-right m-auto" @if($pending !=1) disabled
                                @else @endif>
